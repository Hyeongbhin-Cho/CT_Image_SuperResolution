...
class SRCNNSolver(BaseSolver):
    def __init__(self, config:dict, model: nn.Module,
        ...
        super(SRCNNSolver, self).__init__(config=config, model=model,
        ...
        print("SR CNN solver is loaded")
        
    def train(self):
        assert self.train_loader is not None, "SRCNNSolver need train loader. SRCNNSolver(..., train_loader=<here>)"
            ...
            for epoch in range(1, self.num_epochs + 1):
                ...
                for data in train_pbar:                    
                    ...
                    if total_iters % self.val_iters == 0:
                        assert self.val_loader is not None, "SRCNNSolver need val loader. SRCNNSolver(..., val_loader=<here>)"  
    ...
    def evaluate(self):
        assert self.eval_loader is not None, "SRCNNSolver need eval loader. SRCNNSolver(..., eval_loader=<here>)"
    ...
    def visualize(self):
        assert self.vs_loader is not None, "SRCNNSolver needs vs_loader. SRCNNSolver(..., vs_loader=<here>)"
        self.load_model(self.load_path)
        self.model.eval()
