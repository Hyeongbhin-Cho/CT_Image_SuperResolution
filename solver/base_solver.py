...
class BaseSolver(ABC):
    def __init__(self, config:dict, model: nn.Module,
        ...
        # CUDA
        device_str = config['instrument'].get('device', 'cuda')
        self.device = torch.device(device_str if torch.cuda.is_available() else 'cpu')
        self.model = self.model.to(self.device)
        
        # Multi GPU
        self.multi_gpu = config['instrument'].get('multi_gpu', False)
    ...
    def save_model(self, filename: str=None):
        filename = filename or os.path.join(self.config['save_path'], 'latest.pt')
    ...
    def save_log(self):
        log_path = os.path.join(self.config['save_path'], 'log')
    ...
    def save_fig(self):
        fig_path = os.path.join(self.config['save_path'], 'fig')
        os.makedirs(fig_path, exist_ok=True)
        
        # X-ticks
        record_iters = self.config['train'].get("record_iters", 1)
        x_iters = np.arange(0, len(self.train_losses)) * record_iters
    ...
    def terminate_solver(self):
        ...
        self.save_model()
        self.save_log()
        if self.config["result_fig"]:
            self.save_fig()
        print(f"Model saved to: {self.config['save_path']}")
        print(f"Logs saved to: {os.path.join(self.config['save_path'], 'log')}")
        print(f"Figures saved to: {os.path.join(self.config['save_path'], 'fig')}")
        print("Terminated.")
