...
def compute_measure(x, y, metrics=["psnr", "ssim"], **kwargs):
    data_range = kwargs.get("data_range", 1.0)
    results = {}
    for metric in metrics:
        if metric == "psnr":
            results["psnr"] = compute_PSNR(x, y, data_range=data_range)
        elif metric == "ssim":
            results["ssim"] = compute_SSIM(x, y, data_range=data_range)
        elif metric == "rmse":
            results["rmse"] = compute_RMSE(x, y)
        else:
